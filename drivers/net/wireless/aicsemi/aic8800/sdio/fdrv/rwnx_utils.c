...
extern int aicwf_sdio_get_testmode(void);
extern void aicwf_sdio_get_fw_path(char* fw_path);
extern int testmode;
...
int rwnx_init_aic(struct rwnx_hw *rwnx_hw)
{
	...
	testmode = aicwf_sdio_get_testmode();
	memset(aic_fw_path, 0, 200);
	aicwf_sdio_get_fw_path(aic_fw_path);

...
/* MTK platform power management stub functions */
static inline int hwPowerOn(int id, int vol, char *name)
{
	TOUCH_I("%s: id=%d, vol=%d, name=%s (stub)\n", __func__, id, vol, name);
	return 0;
}

static inline int hwPowerDown(int id, char *name)
{
	TOUCH_I("%s: id=%d, name=%s (stub)\n", __func__, id, name);
	return 0;
}

/* MTK/LGE platform boot mode stub functions */
#ifndef RECOVERY_BOOT
#define RECOVERY_BOOT 0
#endif

#ifndef CHARGER_BOOT
#define CHARGER_BOOT 0
#endif

#ifndef LGE_BOOT_MODE_CHARGERLOGO
#define LGE_BOOT_MODE_CHARGERLOGO 0
#endif

static inline unsigned int get_boot_mode(void)
{
	/* Return normal boot mode as default */
	return 0;
}

static inline unsigned int lge_get_boot_mode(void)
{
	/* Return normal boot mode as default */
	return 0;
}

static inline bool lge_get_factory_boot(void)
{
	/* Return false (not factory boot) as default */
	return false;
}

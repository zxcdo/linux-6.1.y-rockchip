...
static int sdcard_spec_file_read(struct device *dev)
{
	...
	file = filp_open(path[path_idx], O_RDONLY, 0);
	if (IS_ERR(file)) {
		return 0;
	}
	...
	pos = 0;
	ret = kernel_read(file, line, size, &pos);
	filp_close(file, NULL);
...
static void __used read_tune_code(struct device *dev, u8 type)
{
	...
	struct tune_data_sel sel = {
		...
		.m2 = {
			.m2_tune_high_sel_code = {0},
			.m2_tune_low_sel_code = {0},
			.self1_tune_sel_code = {0},
			.self2_tune_sel_code = {0},
			.icr_tune_sel_code = {0},
		}
	};
